...
//NewBIP39 -- creates new BIP39.
func NewBIP39(list Wordlists) (*BIP39, error) {
	var err error
	bip39 := &BIP39{}

	switch list {
	case CHINESE:
		SetWordList(words.Chinese)
	case JAPANESE:
		SetWordList(words.Japanese)
	default:
		SetWordList(words.English)
	}

	if bip39.entropy, err = NewEntropy(128); err != nil {
		return nil, err
	}
	if bip39.mnemonic, err = NewMnemonic(bip39.entropy); err != nil {
		return nil, err
	}
	return bip39, nil
}

// ToString -- returns the mnemonic.
func (bip39 *BIP39) ToString() string {
	return bip39.mnemonic
}

// Seed -- returns the seed of the mnemonic.
// Default passphrase is TREZOR.
func (bip39 *BIP39) Seed() []byte {
	return NewSeed(bip39.mnemonic, "TREZOR")
}
