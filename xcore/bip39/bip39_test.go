...
func TestGetWordList(t *testing.T) {
	assert.Equal(t, words.English, GetWordList())
}
...
func TestPadByteSlice(t *testing.T) {
	assert.Equal(t, []byte{0}, padByteSlice([]byte{}, 1))
	assert.Equal(t, []byte{0, 1}, padByteSlice([]byte{1}, 2))
	assert.Equal(t, []byte{1, 1}, padByteSlice([]byte{1, 1}, 2))
	assert.Equal(t, []byte{1, 1, 1}, padByteSlice([]byte{1, 1, 1}, 2))
}
...
func testEntropyFromMnemonic(t *testing.T, bitSize int) {
	for i := 0; i < 512; i++ {
		...
		assert.Nil(t, err)
		assert.Equal(t, expectedEntropy, actualEntropy)
	}
}
...
func TestBIP39Builder(t *testing.T) {
	...
	entropy1, err := EntropyFromMnemonic(mnemonic)
	assert.Nil(t, err)
	assert.Equal(t, entropy, entropy1)
...
func TestBIP39Example(t *testing.T) {
	// Chinese.
	{
		bip39, err := NewBIP39(CHINESE)
		assert.Nil(t, err)
		t.Logf("%v", bip39.ToString())
		t.Logf("%x", bip39.Seed())
	}

	// English.
	{
		bip39, err := NewBIP39(ENGLISH)
		assert.Nil(t, err)
		t.Logf("%v", bip39.ToString())
		t.Logf("%x", bip39.Seed())
	}
